...
for DATASET_NAME in dataset1 dataset2; do
  ...
  for i in 1 5 10; do
    python merge_gt_to_pred.py \
      ...
      --output ${SUBDATASET_PATH}/annotations/${i}_shot_sam3_conf08_strategy1.json \
      --score-threshold 0.8 \
      --iou-thresh 0.8 \
      --train-val-split \
      --split-strategy strategy1
  done
done
...
for i in 1 5 10; do
  python merge_gt_to_pred.py \
    ...
    --pred-image-dir ${SUBDATASET_PATH}/train \
    --output ${SUBDATASET_PATH}/annotations/${i}_shot_qwen_conf08_strategy1.json \
    --score-threshold 0.8 \
    --iou-thresh 0.8 \
    --train-val-split \
    --split-strategy strategy1 
