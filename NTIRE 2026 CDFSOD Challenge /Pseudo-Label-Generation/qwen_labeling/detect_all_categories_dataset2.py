...
CATEGORIES_PROMPT = """Detect the following category in the image:
[
    {"id": 1, "name": "car"},
]
...
def main():
    ...
    if not image_files:
        print(f"No images found under {path}")
        return
    ...
    for i, img_path in enumerate(image_files):
        ...
        out_json_path = os.path.join(output_dir, stem + ".json")
        if os.path.isfile(out_json_path):
            print(f"[{i + 1}/{len(image_files)}] {os.path.basename(img_path)} (skipped, already annotated)")
            continue
