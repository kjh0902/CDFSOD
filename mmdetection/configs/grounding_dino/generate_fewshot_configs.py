...
DATASETS = {
    ...
    'clipart1k': {
        ...
        'classes': [
            'sheep', 'chair', 'boat', 'bottle', 'diningtable', 'sofa', 'cow', 'motorbike', 'car', 'aeroplane',
            'cat', 'train', 'person', 'bicycle', 'pottedplant', 'bird', 'dog', 'bus', 'tvmonitor', 'horse'
        ]
    },
    'NEU-DET': {
...
CONFIG_TEMPLATE = '''# Auto-generated config for {dataset} {shot}-shot
...
def main():
    os.makedirs('grounding_dino', exist_ok=True)
    for dataset, info in DATASETS.items():
        for shot in SHOTS:
            config_str = CONFIG_TEMPLATE.format(
                ...
                classes=repr(tuple(info['classes']))
            )
            config_name = f'CDFSOD_detection_few-shot_{dataset}_{shot}shot.py'
            config_path = os.path.join('grounding_dino', config_name)
