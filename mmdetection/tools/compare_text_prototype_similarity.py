#!/usr/bin/env python
"""Compare class-name-token prototype cosine similarity from two checkpoints.
...
overlap the class-name span, averages those tokens per class, and prints
class-to-class prototype similarities.
...
DEFAULT_CLASS_NAME_TOKEN_PROTO_CKPT = (
    '/home/aislab5090/CDFSOD/junhyung/grounding_dino_idea/CDFSOD/'
    'mmdetection/work_dirs/NEU-DET_1shot_class_name_token_prototype/'
    'epoch_30.pth')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Compare class-name-token prototype cosine similarities.')
    parser.add_argument('--config', default=str(DEFAULT_CONFIG))
    parser.add_argument('--class-name-ckpt', default=DEFAULT_CLASS_NAME_CKPT)
    parser.add_argument(
        '--class-name-token-prototype-ckpt',
        '--enriched-class-token-ckpt',
        '--class-prototype-ckpt',
        dest='class_name_token_prototype_ckpt',
        default=DEFAULT_CLASS_NAME_TOKEN_PROTO_CKPT)
...
def build_model(config_path, checkpoint_path, caption_file, device):
    ...
    cfg.model.use_class_name_token_prototypes = True
...
def get_raw_and_projected_prototypes(model):
    """Return class-name-token prototypes before/after text_feat_map."""
    with torch.no_grad():
...
def summarize_class_separation(name, class_names, features, print_matrix):
    ...
    print(f'\n[{name}]')
    print(f'class-name-token prototype shape: {tuple(features.shape)}')
    ...
    if print_matrix:
        print('\nclass-by-class prototype cosine matrix:')
...
def compare_same_class_between_checkpoints(name, class_names, left, right):
    ...
    print(f'\n[{name}] same-class prototype cosine between checkpoints')
    for cls, sim in zip(class_names, sims):
        print(f'{cls:18s}: {sim.item():.6f}')
    print(f'mean same-class prototype cosine: {sims.mean().item():.6f}')
...
def main():
    ...
    for path_name, path in [
            ...
            ('class-name-token-prototype checkpoint',
             args.class_name_token_prototype_ckpt),
    ...
    print('class-name-token-prototype checkpoint:',
          args.class_name_token_prototype_ckpt)
    print('device:', device)

    class_name_model = build_model(
        args.config, args.class_name_ckpt, args.caption_file, device)
    class_token_proto_model = build_model(
        args.config, args.class_name_token_prototype_ckpt,
        args.caption_file, device)
    class_names = class_name_model.support_class_names

    name_raw, name_proj = get_raw_and_projected_prototypes(class_name_model)
    proto_raw, proto_proj = get_raw_and_projected_prototypes(
        class_token_proto_model)

    summarize_class_separation(
        'class_name checkpoint / raw BERT class-name-token prototypes',
        class_names, name_raw, args.print_matrix)
    summarize_class_separation(
        'class-name-token-prototype checkpoint / raw BERT prototypes',
    ...
    summarize_class_separation(
        'class-name-token-prototype checkpoint / projected detector '
        'prototypes',
        class_names, proto_proj, args.print_matrix)

    compare_same_class_between_checkpoints(
        'raw BERT class-name-token prototypes', class_names, name_raw,
        proto_raw)
    compare_same_class_between_checkpoints(
    ...
    print('- Each prototype is the mean of all selected class-name token '
          'features from the same class support prompts.')
