...
class PrototypeDDPTests(unittest.TestCase):
    ...
    def test_unused_tail_and_two_ddp_iterations_with_checkpointing(self):
        for freeze, use_checkpoint in ((False, False), (False, True),
                                       (True, False), (True, True)):
            with self.subTest(freeze=freeze, checkpoint=use_checkpoint):
                torch.manual_seed(7)
                ...
                expected_loss = loss_model(features).detach()
                keys = set(model.state_dict())
                if freeze:
                    model._freeze_unused_enhancer_text_parameters()
                torch.testing.assert_close(loss_model(features), expected_loss)
                ...
                try:
                    ...
                    ddp(features).backward()
                    unused = {name for name, p in model.named_parameters()
                              if p.requires_grad and p.grad is None}
                    if not freeze:
                        self.assertTrue(any('text_layers.5.' in n for n in unused))
                        if not use_checkpoint:
                            self.assertTrue(any('out_l_proj' in n for n in unused))
                        with self.assertRaisesRegex(RuntimeError, 'Expected to have finished reduction'):
                            ddp(features)
                    else:
