"""Second-order geometry checks, runnable with CPU PyTorch alone."""
...
spec = importlib.util.spec_from_file_location(
    package.__name__ + '.second_order_etf_loss',
    ROOT / 'mmdet/models/losses/second_order_etf_loss.py')
second = importlib.util.module_from_spec(spec)
spec.loader.exec_module(second)


class SecondOrderGeometryTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_exact_outer_product_mean_without_class_normalization(self):
        ...
        with patch.object(second, 'nearest_etf_loss', wraps=etf.nearest_etf_loss) as loss:
            second.second_order_etf_loss(self.x, self.maps, self.mask)
    ...
    def test_batch_independence_and_gradients_only_for_selected_tokens(self):
        loss = second.second_order_etf_loss(self.x, self.maps, self.mask)
        individual = [second.second_order_etf_loss(
        ...
        self.assertTrue(torch.autograd.gradcheck(
            lambda x: second.second_order_etf_loss(x, self.maps, self.mask), (self.x,)))

    def test_low_precision_autocast(self):
        for dtype in [torch.float16, torch.bfloat16]:
            x = self.x.detach().to(dtype).requires_grad_()
            expected = second.second_order_etf_loss(x.float(), self.maps, self.mask)
            with torch.autocast('cpu', dtype=dtype):
                actual = second.second_order_etf_loss(x, self.maps, self.mask)
    ...
    def test_zero_duplicate_and_collapsed_classes(self):
        for x in [torch.zeros_like(self.x), torch.ones_like(self.x), self.x.detach() * 1e-10]:
            x.requires_grad_()
            loss = second.second_order_etf_loss(x, self.maps, self.mask)
            loss.backward()
            self.assertTrue(torch.isfinite(loss))
            self.assertTrue(torch.isfinite(x.grad).all())
    ...
    def test_invalid_maps_and_mask(self):
        for mapping in [{1: [], 2: [2], 3: [3]}, {1: [-1], 2: [2], 3: [3]},
            ...
            with self.subTest(mapping=mapping), self.assertRaises(ValueError):
                second.second_order_etf_loss(self.x, [mapping, self.maps[1]], self.mask)
        mask = self.mask.clone()
        mask[0, 1] = False
        for invalid in [mask, self.mask.float(), self.mask[:, :-1]]:
            with self.assertRaises(ValueError):
                second.second_order_etf_loss(self.x, self.maps, invalid)
        for eps in [0, -1, float('nan'), float('inf')]:
            with self.assertRaises(ValueError):
                second.second_order_etf_loss(self.x, self.maps, self.mask, eps)
        with self.assertRaises(ValueError):
            second.second_order_etf_loss(self.x, self.maps[:1], self.mask)
        with self.assertRaises(ValueError):
            second.second_order_etf_loss(self.x[:, :, :1], self.maps, self.mask)
